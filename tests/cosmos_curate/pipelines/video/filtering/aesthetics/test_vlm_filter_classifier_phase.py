...
def test_vlm_filter_classifier_requires_at_least_one_config() -> None:
    """Builder must receive at least one config."""
    msg = "At least one of filter_config, first_person_pov_config, or classifier_config"
...
def test_vlm_filter_classifier_build_stages_first_person_pov_only() -> None:
    """First-person POV-only builds VllmPrepStage + VllmCaptionStage + VllmFirstPersonPovFilterStage."""
    stages = build_vllm_filter_classifier_stages(first_person_pov_config=FirstPersonPovFilterConfig())
    assert len(stages) == 3
    assert isinstance(stages[0], VllmPrepStage)
    assert isinstance(stages[1], CuratorStageSpec)
    assert isinstance(stages[1].stage, VllmCaptionStage)
    assert isinstance(stages[2], CuratorStageSpec)
    assert isinstance(stages[2].stage, VllmFirstPersonPovFilterStage)


def test_vlm_filter_classifier_build_stages_all_three() -> None:
    """All configs run semantic filter, then first-person POV filter, then classifier."""
    stages = build_vllm_filter_classifier_stages(
        filter_config=VlmFilterConfig(),
        first_person_pov_config=FirstPersonPovFilterConfig(),
        classifier_config=VideoClassifierConfig(),
    )
    assert len(stages) == 9
    assert isinstance(stages[2], CuratorStageSpec)
    assert isinstance(stages[2].stage, VllmFilteringStage)
    assert isinstance(stages[5], CuratorStageSpec)
    assert isinstance(stages[5].stage, VllmFirstPersonPovFilterStage)
    assert isinstance(stages[8], CuratorStageSpec)
    assert isinstance(stages[8].stage, VllmVideoClassifierStage)
...
def test_first_person_pov_build_stages_openai_endpoint() -> None:
    """OpenAI first-person POV endpoint reuses the filter endpoint config."""
    stages = build_vllm_filter_classifier_stages(
        first_person_pov_config=FirstPersonPovFilterConfig(endpoint="openai", batch_size=6)
    )
    assert len(stages) == 3
    assert isinstance(stages[0], ApiPrepStage)
    assert isinstance(stages[1], OpenAICaptionStage)
...
def test_vlm_filtering_stage_writes_text_contamination_audit_fields() -> None:
    """Semantic text filtering should preserve audit fields for manual review."""
    stage = VllmFilteringStage(model_variant="qwen", user_prompt="post-production text", score_only=True)
    video, clip_idx = _make_video_with_one_clip_one_window()
    result_json = (
        '{"post-production text": "yes", '
        '"text_type": "hud_ui", '
        '"reason": "Quest text is overlaid on top of the gameplay."}'
    )

    stage._process_semantic_clip(
        video,
        clip_idx,
        [(0, result_json)],
    ...
    window = video.clips[clip_idx].filter_windows[0]
    assert "post-production text" in window.caption["qwen_rejection_reasons"]
    assert window.caption["qwen_filter_text_type"] == "hud_ui"
    assert window.caption["qwen_filter_reason"] == "Quest text is overlaid on top of the gameplay."
...
def _make_video_one_clip_two_windows() -> tuple[Video, int]:
    """Minimal Video with one clip and two filter_windows."""
    clip = Clip(
        uuid=uuid.uuid4(),
        source_video="test.mp4",
        span=(0.0, 5.0),
    ...
    clip.filter_windows.append(Window(start_frame=0, end_frame=124))
    clip.filter_windows.append(Window(start_frame=125, end_frame=249))
    video = Video(input_video=pathlib.Path("test.mp4"), clips=[clip])
    return video, 0


def test_first_person_pov_stage_requires_majority_yes() -> None:
    """At threshold .5, one yes and one no is rejected because pass requires > threshold."""
    stage = VllmFirstPersonPovFilterStage(model_variant="qwen", rejection_threshold=0.5)
    video, clip_idx = _make_video_one_clip_two_windows()

    stage._filter_clips(
        video,
        {0: (clip_idx, 0), 1: (clip_idx, 1)},
        enumerate(['{"first_person_pov": "yes"}', '{"first_person_pov": "no"}']),
    )

    assert len(video.clips) == 0
    assert len(video.filtered_clips) == 1
    filtered = video.filtered_clips[0]
    assert filtered.qwen_rejection_stage == "first_person_pov"
    assert filtered.filter_windows[0].caption["qwen_first_person_pov"] == "yes"
    assert filtered.filter_windows[1].caption["qwen_first_person_pov"] == "no"
    assert video.clip_stats.num_filtered_by_qwen_first_person_pov == 1


def test_first_person_pov_stage_keeps_majority_yes() -> None:
    """A clip passes when the yes fraction is greater than the threshold."""
    stage = VllmFirstPersonPovFilterStage(model_variant="qwen", rejection_threshold=0.5)
    video, clip_idx = _make_video_with_one_clip_one_window()

    stage._filter_clips(video, {0: (clip_idx, 0)}, enumerate(['{"first_person_pov": "yes"}']))

    assert len(video.clips) == 1
    assert len(video.filtered_clips) == 0
    assert video.clips[0].filter_windows[0].caption["qwen_first_person_pov"] == "yes"


def test_first_person_pov_stage_malformed_filters_clip() -> None:
    """Malformed first-person POV output should reject the clip and store an isolated window error."""
    stage = VllmFirstPersonPovFilterStage(model_variant="qwen")
    video, clip_idx = _make_video_with_one_clip_one_window()
    ...
    assert len(video.clips) == 0
    assert len(video.filtered_clips) == 1
    assert video.filtered_clips[0].qwen_rejection_stage == "first_person_pov"
    assert video.filtered_clips[0].filter_windows[0].errors.get("qwen_first_person_pov") == "malformed_model_output"


def test_first_person_pov_stage_score_only_keeps_clip() -> None:
    """Score-only first-person POV filtering writes metadata without moving clips to filtered_clips."""
    stage = VllmFirstPersonPovFilterStage(model_variant="qwen", score_only=True)
    video, clip_idx = _make_video_with_one_clip_one_window()

    stage._filter_clips(video, {0: (clip_idx, 0)}, enumerate(['{"first_person_pov": "no"}']))

    assert len(video.clips) == 1
    assert len(video.filtered_clips) == 0
    assert video.clips[0].filter_windows[0].caption["qwen_first_person_pov"] == "no"
