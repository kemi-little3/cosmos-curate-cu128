...
def _make_video_with_encoded_clip(mp4_bytes: bytes = b"fake-mp4") -> Video:
    clip = Clip(uuid=uuid4(), source_video="source.mp4", span=(0.0, 1.0), encoded_data=mp4_bytes)
    video = Video(input_video=Path("source.mp4"))
    video.clips.append(clip)
    return video
...
@patch("cosmos_curate.pipelines.video.captioning.gemini_caption_stage.windowing_utils.make_windows_for_video")
@patch("cosmos_curate.pipelines.video.captioning.gemini_caption_stage.windowing_utils.get_frame_count")
def test_api_prep_stage_filter_windows_use_one_whole_clip_window(
    mock_get_frame_count: MagicMock,
    mock_make_windows: MagicMock,
) -> None:
    """Filter prep should create one whole-clip filter window instead of caption windows."""
    window_config = WindowConfig(window_size=10)
    stage = ApiPrepStage(window_config, use_filter_windows=True, whole_clip_filter_windows=True)
    video = _make_video_with_encoded_clip(b"fake-mp4")
    mock_get_frame_count.return_value = 25

    stage._prep_windows(video)

    mock_make_windows.assert_not_called()
    clip = video.clips[0]
    assert clip.windows == []
    assert len(clip.filter_windows) == 1
    assert clip.filter_windows[0].start_frame == 0
    assert clip.filter_windows[0].end_frame == 24
    assert bytes(clip.filter_windows[0].mp4_bytes.resolve()) == b"fake-mp4"
