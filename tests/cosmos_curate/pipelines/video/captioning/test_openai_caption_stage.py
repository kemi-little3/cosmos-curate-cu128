...
def test_generate_caption_passes_extra_body(monkeypatch: pytest.MonkeyPatch) -> None:
    """Provider-specific OpenAI-compatible parameters should be forwarded."""
    extra_body = {"top_k": 20, "chat_template_kwargs": {"enable_thinking": False}}
    stage = _make_stage(monkeypatch, extra_body=extra_body)

    create = AsyncMock(return_value=_FakeResponse([_FakeChoice("caption")]))
    _attach_openai_async_client(stage, create)

    stage.process_data([_make_task(b"\x00\x01")])

    assert create.call_args.kwargs["extra_body"] == extra_body
