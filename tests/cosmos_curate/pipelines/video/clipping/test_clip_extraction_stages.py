...
class TestTargetTranscodeFormat:
    """Tests for target FPS/resolution transcoding options."""

    def test_build_video_filter_center_crop(self) -> None:
        ...
        assert (
            stage._build_video_filter()
            == "fps=16,crop='min(iw,ih*1280/720)':'min(ih,iw*720/1280)',scale=1280:720,setsar=1"
        )

    @pytest.mark.parametrize(
        ("width", "height", "too_small"),
        [
            (1280, 720, False),
            (1920, 1080, False),
            (1024, 768, True),
    ...
    def test_resolution_too_small(self, width: int, height: int, *, too_small: bool) -> None:
        stage = ClipTranscodingStage(target_width=1280, target_height=720)
        video = Video(
            input_video=pathlib.Path("test.mp4"),
            metadata=VideoMetadata(width=width, height=height),
        )

        assert stage._resolution_too_small(video) is too_small

    def test_filtered_nvenc_transcode_does_not_request_hardware_decode(self, tmp_path: pathlib.Path) -> None:
        stage = ClipTranscodingStage(
            encoder="h264_nvenc",
            use_hwaccel=True,
            target_fps=16,
        ...
        clip = Clip(uuid=uuid.uuid4(), source_video="test.mp4", span=(0.0, 10.0))
        ...
        assert captured_cmd[captured_cmd.index("-c:v") + 1] == "h264_nvenc"
        assert captured_cmd[captured_cmd.index("-vf") + 1] == stage._build_video_filter()
