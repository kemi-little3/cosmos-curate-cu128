...
def test_predict_windows_by_frame_count_summary() -> None:
    """Summary reports per-frame-count window totals from processed clip chunks."""
    clip_chunk = {
        "clips": ["clip-a"],
        "filtered_clips": ["clip-a"],
        "all_windows": {
            "clip-a": {
                "0_80": "caption 81",
                "0_320": "caption 321 a",
                "321_641": "caption 321 b",
            }
        },
    }
    processed = {
        "test.ts": type(
            ...
            (),
            {
                "video_metadata": {"video_uuid": "video-uuid", "duration": 10.0},
                "clip_chunks": [clip_chunk],
            },
        )()
    }

    with (
        patch("cosmos_curate.pipelines.video.read_write.summary_writers.storage_utils.get_storage_client"),
        patch("cosmos_curate.pipelines.video.read_write.summary_writers.get_files_relative", return_value=["test.ts"]),
        patch(
            "cosmos_curate.pipelines.video.read_write.summary_writers._read_all_video_metadata_parallel",
            return_value=processed,
        ),
        patch("cosmos_curate.pipelines.video.read_write.summary_writers.write_json") as mock_write_json,
    ):
        write_split_summary(
            input_path="/in",
            input_videos_relative=["test.ts"],
    ...
    summary = mock_write_json.call_args.args[0]
    assert summary["num_predict_windows_by_frame_count"] == {"81": 1, "321": 2}
