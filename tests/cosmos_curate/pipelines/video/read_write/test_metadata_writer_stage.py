...
def test_cosmos_predict_paths_are_grouped_by_frame_count_when_configured(tmp_path: Path) -> None:
    """Grouped mode routes per-window dataset assets through frame-count subdirectories."""
    input_dir = tmp_path / "input"
    output_dir = tmp_path / "output"
    input_dir.mkdir()
    stage = _create_stage(
        output_dir,
        input_dir,
        generate_cosmos_predict_dataset="predict2",
        frame_number_output_subdirs={81: "frames_81", 321: "frames_321"},
    )
    clip_uuid = uuid.UUID("00000000-0000-0000-0000-000000000123")

    video_uri = stage._get_cosmos_predict_uri(
        clip_uuid,
        (0, 80),
        stage._get_cosmos_predict_window_output_path(81, "videos"),
        "mp4",
    )
    meta_uri = stage._get_cosmos_predict_uri(
        clip_uuid,
        (0, 320),
        stage._get_cosmos_predict_window_output_path(321, "metas"),
        "txt",
    )

    assert str(video_uri).endswith(
        "cosmos_predict2_video2world_dataset/frames_81/videos/00000000-0000-0000-0000-000000000123_0_80.mp4"
    )
    assert str(meta_uri).endswith(
        "cosmos_predict2_video2world_dataset/frames_321/metas/00000000-0000-0000-0000-000000000123_0_320.txt"
    )


def test_cosmos_predict_paths_keep_legacy_layout_without_grouping(tmp_path: Path) -> None:
    """Without grouped mode, per-window dataset assets keep the legacy flat layout."""
    input_dir = tmp_path / "input"
    output_dir = tmp_path / "output"
    input_dir.mkdir()
    stage = _create_stage(output_dir, input_dir, generate_cosmos_predict_dataset="predict2")

    path = stage._get_cosmos_predict_window_output_path(81, "videos")

    assert str(path).endswith("cosmos_predict2_video2world_dataset/videos")
