...
def test_get_identity_mounts_skips_root_uid(tmp_path: Path) -> None:
    """Root already resolves in the image and must not mount over /etc/passwd."""
    with patch("cosmos_curate.client.local_cli.launch_local.os.getuid", return_value=0):
        assert _get_identity_mounts(tmp_path) == []
    assert not (tmp_path / "etc").exists()
...
def test_get_pixi_mount_strings_mounts_only_pixi_directory(tmp_path: Path) -> None:
    """Shared-storage Docker supports directory mounts; avoid file-level pixi mounts."""
    (tmp_path / ".pixi").mkdir()
    (tmp_path / "pixi.toml").write_text("[project]\n")
    (tmp_path / "pixi.lock").write_text("version = 6\n")
    opts = LaunchDocker(
        image_label="cosmos-curate:slim",
        curator_path=None,
        pixi_path=str(tmp_path),
        mount_xenna=False,
        command="true",
        gpus=None,
    ...
    result = _get_pixi_mount_strings(opts)

    assert result == ["-v", f"{(tmp_path / '.pixi').absolute()}:/opt/cosmos-curate/.pixi"]
    assert "pixi.toml" not in " ".join(result)
    assert "pixi.lock" not in " ".join(result)
