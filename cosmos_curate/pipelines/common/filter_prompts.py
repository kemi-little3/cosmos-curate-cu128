...
FILTER_CRITERIA: dict[str, list[str]] = {
    ...
    "text-contamination": ["post-production text"],
}
...
_CAMERA_CHARACTER_COUPLING_PROMPT = """Determine whether the camera in this video is tightly coupled to the motion of a primary character.

"Coupled" means:
- When the character moves (walks, runs, turns), the camera moves correspondingly (translation and/or rotation tracks the character)
- When the character rotates their viewpoint or body, the camera follows the rotation
- This includes: first-person POV (camera = character's eyes), third-person follow camera that tracks behind/around a single character, over-the-shoulder cameras locked to the character

"NOT coupled" means any of:
- Camera pans, rotates, or translates while the character stays still
- Character walks, runs, or turns but the camera does not follow
- Camera rotates but the character's body or facing direction does not rotate
- Static or fixed camera observing a scene independent of any character (overhead map view, side-scrolling camera, isometric strategy view, surveillance)
- Multiple characters where the camera does not follow any specific one
- Cinematic cutaways or scripted camera movements not driven by player input

Answer "yes" if the camera is tightly coupled to a primary character's motion.
Answer "no" otherwise.

Answer format:
{
"camera_character_coupled": "yes" or "no"
}"""

_TEXT_CONTAMINATION_PROMPT = """You are checking whether this game video clip contains text overlays that should cause the clip to be rejected from a clean world-model training dataset.

Answer "yes" for text that belongs to overlays, UI, HUD, menus, subtitles, watermarks, chat, notifications, title cards, loading screens, edited annotations, or other non-world visual layers.

Answer "no" for text physically present inside the 3D world, such as signs, posters, billboards, labels on objects, storefront text, or environmental writing.

If unsure, answer "yes" only when the text is prominent enough to affect training.

Return exactly JSON:
{
"post-production text": "yes" or "no",
"text_type": "subtitle|watermark|hud_ui|menu|chat|title_card|edited_annotation|scene_text|none|uncertain",
"reason": "short reason"
}"""
...
def get_media_filter_prompt(
    ...
    *,
    ...
    if prompt_variant == "text-contamination":
        if verbose:
            logger.debug(f"{log_label} prompt: {_TEXT_CONTAMINATION_PROMPT}")
        return _TEXT_CONTAMINATION_PROMPT
...
def get_camera_character_coupling_prompt(*, verbose: bool = False) -> str:
    """Return the video prompt for camera-character motion coupling filtering."""
    if verbose:
        logger.debug(f"Camera-character coupling filtering prompt: {_CAMERA_CHARACTER_COUPLING_PROMPT}")
    return _CAMERA_CHARACTER_COUPLING_PROMPT
