...
CAMERA_CHARACTER_COUPLING_KEY = "camera_character_coupled"
SEMANTIC_AUDIT_KEYS = ("text_type", "reason")
...
def evaluate_semantic_window_results(
    ...
    *,
    ...
    score_only: bool,
) -> tuple[bool, set[str], dict[int, dict[str, str]], dict[int, str], dict[int, dict[str, str]]]:
    ...
    per_window_audit: dict[int, dict[str, str]] = {}
    ...
    for window_idx, result in window_results:
        ...
        window_audit = {
            audit_key: filtering_dict[audit_key]
            for audit_key in SEMANTIC_AUDIT_KEYS
            if isinstance(filtering_dict.get(audit_key), str)
        }
        if window_audit:
            per_window_audit[window_idx] = window_audit
    ...
    return clip_should_pass, all_issues, per_window_reasons, per_window_errors, per_window_audit


def evaluate_camera_character_coupling_window_results(
    window_results: list[tuple[int, str]],
    *,
    rejection_threshold: float,
    score_only: bool,
) -> tuple[bool, dict[int, str], dict[int, str], float]:
    """Evaluate camera-character coupling captions and return pass/fail plus per-window values."""
    per_window_values: dict[int, str] = {}
    per_window_errors: dict[int, str] = {}
    yes_windows = 0
    ...
    for window_idx, result in window_results:
        ...
        if filtering_dict is None:
            per_window_errors[window_idx] = MALFORMED_MODEL_OUTPUT
            per_window_values[window_idx] = "no"
            continue
        value = filtering_dict.get(CAMERA_CHARACTER_COUPLING_KEY, "no")
        if value not in {"yes", "no"}:
            per_window_errors[window_idx] = MALFORMED_MODEL_OUTPUT
            value = "no"
        per_window_values[window_idx] = value
        if value == "yes":
            yes_windows += 1

    yes_fraction = yes_windows / len(window_results) if window_results else 0.0
    clip_should_pass = score_only or (bool(window_results) and yes_fraction > rejection_threshold)
    return clip_should_pass, per_window_values, per_window_errors, yes_fraction
