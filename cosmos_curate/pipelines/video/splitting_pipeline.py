...
from cosmos_curate.pipelines.video.filtering.aesthetics.aesthetics_builders import (
    ...
    CameraCharacterCouplingFilterConfig,
...
def _parse_csv_ints(raw: str, flag_name: str) -> tuple[int, ...]:
    values = tuple(int(part.strip()) for part in raw.split(",") if part.strip())
    if not values or any(value <= 0 for value in values):
        msg = f"{flag_name} must contain positive integers"
        raise ValueError(msg)
    if len(set(values)) != len(values):
        msg = f"{flag_name} must contain unique values"
        raise ValueError(msg)
    return values


def _parse_csv_floats(raw: str, flag_name: str) -> tuple[float, ...]:
    values = tuple(float(part.strip()) for part in raw.split(",") if part.strip())
    if not values or any(value <= 0 or not math.isfinite(value) for value in values):
        msg = f"{flag_name} must contain positive finite numbers"
        raise ValueError(msg)
    return values


def _build_weighted_frame_window_config(args: argparse.Namespace) -> WeightedFrameWindowConfig | None:
    if args.frame_number_choices is None:
        if args.frame_number_weights is not None or args.frame_number_output_subdirs is not None:
            msg = "--frame-number-weights and --frame-number-output-subdirs require --frame-number-choices"
            raise ValueError(msg)
        return None
    if args.frame_number is not None:
        msg = "--frame-number and --frame-number-choices are mutually exclusive"
        raise ValueError(msg)
    if args.frame_number_weights is None:
        msg = "--frame-number-weights is required when --frame-number-choices is set"
        raise ValueError(msg)
    choices = _parse_csv_ints(args.frame_number_choices, "--frame-number-choices")
    weights = _parse_csv_floats(args.frame_number_weights, "--frame-number-weights")
    if len(choices) != len(weights):
        msg = "--frame-number-choices and --frame-number-weights must have the same length"
        raise ValueError(msg)
    return WeightedFrameWindowConfig(choices=choices, weights=weights, random_seed=args.frame_number_random_seed)


def _parse_frame_number_output_subdirs(choices: tuple[int, ...], raw_subdirs: str | None) -> dict[int, str]:
    if raw_subdirs is None:
        return {choice: f"frames_{choice}" for choice in choices}
    subdirs = tuple(part.strip() for part in raw_subdirs.split(",") if part.strip())
    if len(subdirs) != len(choices):
        msg = "--frame-number-output-subdirs must have the same length as --frame-number-choices"
        raise ValueError(msg)
    for subdir in subdirs:
        path = pathlib.PurePosixPath(subdir)
        if path.is_absolute() or len(path.parts) != 1 or ".." in path.parts:
            msg = "--frame-number-output-subdirs entries must be relative path components"
            raise ValueError(msg)
    return dict(zip(choices, subdirs, strict=True))
...
def _assemble_stages(  # noqa: C901, PLR0912, PLR0915
    ...
    stages.extend(
        build_transcode_stages(
            TranscodeConfig(
                ...
                target_fps=args.transcode_target_fps,
                target_width=args.transcode_target_width,
                target_height=args.transcode_target_height,
                resize_mode=args.transcode_resize_mode,
                verbose=args.verbose,
                perf_profile=args.perf_profile,
            )
        )
    ...
    camera_character_coupling_filter_num_gpus = _clamp_num_gpus_for_qwen3_235b(
        args.vlm_filter_model_variant, args.vlm_filter_num_gpus
    )
    ...
    camera_character_coupling_filter_cfg = (
        CameraCharacterCouplingFilterConfig(
            score_only=args.camera_character_coupling_filter == "score-only",
            model_variant=args.vlm_filter_model_variant,
            rejection_threshold=args.camera_character_coupling_threshold,
            batch_size=args.vlm_filter_batch_size,
            fp8_enable=args.vlm_filter_fp8_enable,
            max_output_tokens=args.vlm_filter_max_output_tokens,
            num_gpus=camera_character_coupling_filter_num_gpus,
            use_mmcache=args.qwen_use_vllm_mmcache,
            sampling_fps=args.captioning_sampling_fps,
            window_size=args.captioning_window_size,
            remainder_threshold=args.captioning_remainder_threshold,
        ...
        if args.camera_character_coupling_filter != "disable"
        else None
    )
    ...
    if (
        vlm_filter_cfg is not None
        or camera_character_coupling_filter_cfg is not None
        or video_classifier_cfg is not None
    ):
        stages.extend(
            build_vllm_filter_classifier_stages(
                filter_config=vlm_filter_cfg,
                camera_character_coupling_config=camera_character_coupling_filter_cfg,
    ...
    if args.generate_captions:
        ...
        if caption_algo == "gemini":
            backend = GeminiConfig(
                ...
                num_cpus_for_prepare=args.vllm_prepare_num_cpus_per_worker,
                num_workers_per_node=args.api_caption_num_workers_per_node,
                num_cpus_per_worker=args.api_caption_cpus_per_worker,
            )
            ...
            backend = OpenAIConfig(
                ...
                num_workers_per_node=args.api_caption_num_workers_per_node,
                num_cpus_per_worker=args.api_caption_cpus_per_worker,
                extra_body=QWEN36_OPENAI_EXTRA_BODY,
            )
    ...
    # --- ViPE pose estimation (optional) ---
    if args.enable_vipe_pose:
        if not args.vipe_python:
            msg = "--vipe-python is required when --enable-vipe-pose is set"
            raise ValueError(msg)
        stages.extend(
            build_vipe_pose_stages(
                VipePoseConfig(
    ...
    # --- Output (always) ---
    weighted_frame_output_subdirs: dict[int, str] = {}
    weighted_frame_window_config_for_output = _build_weighted_frame_window_config(args)
    if weighted_frame_window_config_for_output is not None:
        weighted_frame_output_subdirs = _parse_frame_number_output_subdirs(
            weighted_frame_window_config_for_output.choices,
            args.frame_number_output_subdirs,
        )
    stages.extend(
        build_output_stages(
            OutputConfig(
...
def _split(args: argparse.Namespace) -> None:  # noqa: C901, PLR0912, PLR0915
    ...
    if args.frame_number is not None and args.frame_number <= 0:
        msg = f"--frame-number must be positive, got {args.frame_number}"
        raise ValueError(msg)
    weighted_frame_window_config = _build_weighted_frame_window_config(args)
    if weighted_frame_window_config is not None:
        _parse_frame_number_output_subdirs(weighted_frame_window_config.choices, args.frame_number_output_subdirs)
    if args.transcode_target_fps is not None and args.transcode_target_fps <= 0:
        msg = f"--transcode-target-fps must be positive, got {args.transcode_target_fps}"
        raise ValueError(msg)
    if (args.transcode_target_width is None) != (args.transcode_target_height is None):
        msg = "--transcode-target-width and --transcode-target-height must be set together"
        raise ValueError(msg)
    if args.transcode_target_width is not None and args.transcode_target_width <= 0:
        msg = f"--transcode-target-width must be positive, got {args.transcode_target_width}"
        raise ValueError(msg)
    if args.transcode_target_height is not None and args.transcode_target_height <= 0:
        msg = f"--transcode-target-height must be positive, got {args.transcode_target_height}"
        raise ValueError(msg)
...
def _setup_parser(parser: argparse.ArgumentParser) -> None:  # noqa: PLR0915
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Python interpreter from the external ViPE environment used to run the pose adapter.",
    )
    parser.add_argument(
        "--vipe-adapter-script",
        type=str,
        default="/data1/hexuming/pose_estimation/adapter/scripts/run_adapter.py",
        help="Path to the external pose adapter dispatcher script.",
    )
    parser.add_argument(
        "--vipe-fail-policy",
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Number of Ray GPUs to reserve for each ViPE pose worker.",
    )
    parser.add_argument(
        "--vipe-num-workers",
        type=int,
        default=0,
        help=(
            "Fixed number of ViPE pose workers per node. "
            "0 (default) uses xenna autoscaling. "
    ...
    parser.add_argument(
        "--transcode-target-fps",
        type=float,
        default=None,
        help="If set, force transcoded clips to this constant frame rate.",
    )
    parser.add_argument(
        "--transcode-target-width",
        type=int,
        default=None,
        help="If set with --transcode-target-height, force transcoded clips to this width.",
    )
    parser.add_argument(
        "--transcode-target-height",
        type=int,
        default=None,
        help="If set with --transcode-target-width, force transcoded clips to this height.",
    )
    parser.add_argument(
        "--transcode-resize-mode",
        type=str,
        default="center-crop",
        choices=["center-crop"],
        help="Resize mode for target dimensions. center-crop preserves aspect ratio and never pads.",
    )
    parser.add_argument(
        "--clip-re-chunk-size",
        type=int,
    ...
    parser.add_argument(
        "--camera-character-coupling-filter",
        dest="camera_character_coupling_filter",
        choices=["enable", "disable", "score-only"],
        default="disable",
        help=(
            "Whether to enable camera-character motion coupling filtering for video clips. "
            "Uses the VLM filter model/backend settings to judge whether the camera tracks "
            "a primary character's motion (first-person POV, third-person follow, "
            "over-the-shoulder), and rejects decoupled clips (camera-character motion mismatch, "
            "fixed/independent camera, multi-character scenes). "
            "enable: keep clips whose windows are mostly camera-character coupled; "
            "disable: do not run; "
            "score-only: write coupling scores without filtering clips."
        ),
    )
    parser.add_argument(
        "--camera-character-coupling-threshold",
        dest="camera_character_coupling_threshold",
        type=float,
        default=0.5,
        help=(
            "Minimum fraction threshold for camera-character coupled windows. "
            "A clip passes only when yes_fraction is greater than this value."
        ),
    )
    parser.add_argument(
        "--video-classifier",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Strict output window frame count. If set, overrides --captioning-window-size "
            "and drops trailing incomplete windows."
        ),
    )
    parser.add_argument(
        "--frame-number-choices",
        type=str,
        default=None,
        help="Comma-separated strict frame-window choices for weighted per-clip selection, e.g. 81,321,641,961.",
    )
    parser.add_argument(
        "--frame-number-weights",
        type=str,
        default=None,
        help="Comma-separated positive weights matching --frame-number-choices, e.g. 12,3,1.5,1.",
    )
    parser.add_argument(
        "--frame-number-random-seed",
        type=int,
        default=0,
        help="Seed for deterministic per-clip weighted frame-window selection.",
    )
    parser.add_argument(
        "--frame-number-output-subdirs",
        type=str,
        default=None,
        help="Comma-separated output subdirectories matching --frame-number-choices. Defaults to frames_<N>.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Batch size for T5 caption encoding. 0 keeps the built-in default.",
    )
    parser.add_argument(
        "--t5-worker-max-lifetime-m",
        type=int,
        default=-1,
        help="Maximum lifetime in minutes for T5 workers. -1 keeps the pipeline default; 0 disables lifetime-based restarts.",
    )
    parser.add_argument(
        "--t5-variant",
        type=str,
        default="fp32",
        choices=["fp32", "fp16"],
        help="T5 encoder variant for caption embeddings.",
    )
    parser.add_argument(
        "--api-caption-num-workers-per-node",
        type=int,
        default=4,
        help="Number of API caption actors per node for OpenAI and Gemini captioning stages.",
    )
    parser.add_argument(
        "--api-caption-cpus-per-worker",
        type=float,
        default=0.25,
        help="Ray CPU resources reserved by each OpenAI/Gemini API caption actor.",
    )
    parser.add_argument(
