...
class ClipWriterStage(CuratorStage):
    ...
    @staticmethod
    def get_output_path_per_window_poses(output_path: str) -> str:
        """Get path to store per-window camera pose data."""
        return ClipWriterStage._get_output_path(
            ClipWriterStage.get_output_path_cosmos_predict_dataset(output_path),
            "poses",
        )
    ...
    def _get_cosmos_predict_window_output_path(self, frame_count: int, leaf_dir: str) -> str:
        """Return dataset output path for a per-window artifact."""
        dataset_root = self.get_output_path_cosmos_predict_dataset(self._output_path)
        subdir = self._frame_number_output_subdirs.get(frame_count)
        if subdir is not None:
            return self._get_output_path(dataset_root, f"{subdir}/{leaf_dir}")
        return self._get_output_path(dataset_root, leaf_dir)

    def _get_cosmos_predict_pose_uri(
        self,
        video_span_uuid: uuid.UUID,
        window: tuple[int, int],
        file_name: str,
    ) -> storage_client.StoragePrefix | pathlib.Path:
        sample_id = f"{video_span_uuid}_{window[0]}_{window[1]}"
        frame_count = window[1] - window[0] + 1
        return get_full_path(self._get_cosmos_predict_window_output_path(frame_count, "poses"), sample_id, file_name)
    ...
    def _make_clip_metadata(  # noqa: C901, PLR0912, PLR0915
        ...
        for window in clip.filter_windows:
            ...
            if "qwen_filter_text_type" in window.caption:
                curr_filter_window["qwen_filter_text_type"] = window.caption["qwen_filter_text_type"]
            if "qwen_filter_reason" in window.caption:
                curr_filter_window["qwen_filter_reason"] = window.caption["qwen_filter_reason"]
            window_key = (window.start_frame, window.end_frame)
            if window_key in clip.qwen_camera_character_coupled:
                curr_filter_window["qwen_camera_character_coupled"] = clip.qwen_camera_character_coupled[window_key]
            elif "qwen_camera_character_coupled" in window.caption:
                curr_filter_window["qwen_camera_character_coupled"] = window.caption["qwen_camera_character_coupled"]
            if window_key in clip.qwen_camera_character_coupled_errors:
                curr_filter_window.setdefault("errors", {})["qwen_camera_character_coupled"] = (
                    clip.qwen_camera_character_coupled_errors[window_key]
                )
            if window.errors:
                curr_filter_window.setdefault("errors", {}).update(window.errors)
        ...
        for window in clip.windows:
            ...
            window_key = (window.start_frame, window.end_frame)
            if window_key in clip.qwen_camera_character_coupled:
                curr_window["qwen_camera_character_coupled"] = clip.qwen_camera_character_coupled[window_key]
            elif "qwen_camera_character_coupled" in window.caption:
                curr_window["qwen_camera_character_coupled"] = window.caption["qwen_camera_character_coupled"]
            if window_key in clip.qwen_camera_character_coupled_errors:
                curr_window.setdefault("errors", {})["qwen_camera_character_coupled"] = (
                    clip.qwen_camera_character_coupled_errors[window_key]
                )
            elif "qwen_camera_character_coupled" in window.errors:
                curr_window.setdefault("errors", {})["qwen_camera_character_coupled"] = window.errors[
                    "qwen_camera_character_coupled"
                ]
            if window.pose_status is not None:
                curr_window["pose_status"] = window.pose_status
                curr_window["pose_warning_count"] = len(
                    ((window.pose_meta or {}).get("quality") or {}).get("warnings", [])
                )
                if window.pose_status != "error":
                    curr_window["pose_path"] = str(
                        get_full_path(
                            self.get_output_path_per_window_poses(self._output_path),
                            f"{clip.uuid}_{window.start_frame}_{window.end_frame}",
                        )
                    )
                if window.pose_error:
                    curr_window["pose_error"] = window.pose_error
    ...
    def _write_video_metadata(self, video: Video) -> None:  # noqa: C901
        ...
        data = {
            ...
            "num_clips_filtered_by_resolution": video.clip_stats.num_filtered_by_resolution,
            ...
            "num_clips_filtered_by_qwen_camera_character_coupled": (
                video.clip_stats.num_filtered_by_qwen_camera_character_coupled
            ),
            ...
            for window in clip.windows:
                ...
                for model in self._enhanced_caption_models:
                    if model in window.enhanced_caption:
                        ...
                        break
                if window.pose_status is not None:
                    data.setdefault("all_windows_pose_status", {}).setdefault(clip_uuid, {})[window_key] = (
                        window.pose_status
                    )
    ...
    def _write_per_window_data(self, clip: Clip) -> None:
        ...
        for window in clip.windows:
            ...
            if self._vipe_pose_enabled and not has_pose and self._vipe_fail_policy == "skip-window":
                logger.error(
                    f"Clip {clip.uuid} window [{window.start_frame}, {window.end_frame}] "
                    f"from {clip.source_video} has no successful pose, skip uploading to dataset",
                )
                continue
            ...
            dest_video = self._get_cosmos_predict_uri(
                clip.uuid,
                (window.start_frame, window.end_frame),
                self._get_cosmos_predict_window_output_path(frame_count, "videos"),
                "mp4",
            )
            self._write_data(
                mp4_data,
            ...
            dest_meta = self._get_cosmos_predict_uri(
                clip.uuid,
                (window.start_frame, window.end_frame),
                self._get_cosmos_predict_window_output_path(frame_count, "metas"),
                "txt",
            )
    ...
    def _write_npy_data(
        ...
        self._write_data(buffer.getvalue(), dest, desc, source_video)

    def _write_per_window_pose_data(self, clip: Clip, window: Window) -> None:
        if window.pose_intrinsics is None or window.pose_c2w is None or window.pose_relative is None:
            logger.error(
                f"Clip {clip.uuid} window [{window.start_frame}, {window.end_frame}] "
                f"from {clip.source_video} has incomplete pose data",
            )
            return
        window_tuple = (window.start_frame, window.end_frame)
        self._write_npy_data(
            window.pose_intrinsics,
            self._get_cosmos_predict_pose_uri(clip.uuid, window_tuple, "intrinsics.npy"),
            f"dataset pose intrinsics {clip.uuid} {window.start_frame}_{window.end_frame}",
            clip.source_video,
        )
        self._write_npy_data(
            window.pose_c2w,
            self._get_cosmos_predict_pose_uri(clip.uuid, window_tuple, "poses.npy"),
            f"dataset poses {clip.uuid} {window.start_frame}_{window.end_frame}",
            clip.source_video,
        )
        self._write_npy_data(
            window.pose_relative,
            self._get_cosmos_predict_pose_uri(clip.uuid, window_tuple, "relative_poses.npy"),
            f"dataset relative poses {clip.uuid} {window.start_frame}_{window.end_frame}",
            clip.source_video,
        )
        self._write_json_data(
            window.pose_meta or {},
            self._get_cosmos_predict_pose_uri(clip.uuid, window_tuple, "pose_meta.json"),
            f"dataset pose metadata {clip.uuid} {window.start_frame}_{window.end_frame}",
            clip.source_video,
        )
