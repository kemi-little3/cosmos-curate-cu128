...
def _write_split_result_summary(  # noqa: PLR0912, PLR0913, PLR0915, C901
    input_path: str,
    input_videos_relative: list[str],
    ...
    *,
    ...
    clip_stats_keys = [
        ...
        "num_clips_filtered_by_resolution",
    ...
    summary_data: dict[str, Any] = {
        ...
        "total_windows_with_pose": 0,
        "total_windows_pose_failed": 0,
        "num_predict_windows_by_frame_count": {},
    }
    ...
    for input_video, data in all_video_data.items():
        ...
        summary_data[input_video]["windows_with_pose"] = 0
        summary_data[input_video]["windows_pose_failed"] = 0
        for clip_chunk in data.clip_chunks:
            ...
            for clip_pose_status in clip_chunk.get("all_windows_pose_status", {}).values():
                for pose_status in clip_pose_status.values():
                    if pose_status in {"ok", "degraded"}:
                        summary_data[input_video]["windows_with_pose"] += 1
                        summary_data["total_windows_with_pose"] += 1
                    elif pose_status == "error":
                        summary_data[input_video]["windows_pose_failed"] += 1
                        summary_data["total_windows_pose_failed"] += 1
            for windows_by_clip in clip_chunk.get("all_windows", {}).values():
                for window_key in windows_by_clip:
                    try:
                        start_raw, end_raw = window_key.split("_", 1)
                        frame_count = int(end_raw) - int(start_raw) + 1
                    except (ValueError, AttributeError):
                        logger.warning(f"Skipping malformed window key in summary: {window_key!r}")
                        continue
                    frame_key = str(frame_count)
                    by_frame_count = summary_data["num_predict_windows_by_frame_count"]
                    by_frame_count[frame_key] = by_frame_count.get(frame_key, 0) + 1
        summary_data["total_video_duration"] += data.video_metadata.get("duration", 0)

    summary_data["num_predict_windows_by_frame_count"] = dict(
        sorted(summary_data["num_predict_windows_by_frame_count"].items(), key=lambda item: int(item[0]))
    )
