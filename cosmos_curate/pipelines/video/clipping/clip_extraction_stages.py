...
def chunk_tasks(tasks: list[SplitPipeTask], num_clips_per_chunk: int, *, verbose: bool = False) -> list[SplitPipeTask]:
    ...
    for task in tasks:
        ...
        if not primary_clips and any(v.filtered_clips for v in task.videos):
            ...
            if verbose:
                logger.info(f"Spawning filtered-only subtask for session {task.session_id}.")
            output_tasks.append(subtask)
            logger.info(f"Creating 1 filtered-only task for downstream from session {task.session_id}.")
            continue
        primary_chunks = list(
...
class ClipTranscodingStage(CuratorStage):
    ...
    def __init__(  # noqa: PLR0913
        ...
        *,
        ...
        Args:
            ...
            target_fps: If set, force transcoded clips to this constant frame rate.
            target_width: If set with target_height, force transcoded clips to this width.
            target_height: If set with target_width, force transcoded clips to this height.
            resize_mode: Resize mode for target dimensions. Only center-crop is supported.
            verbose: Whether to print verbose logs.
        ...
        if target_fps is not None and target_fps <= 0:
            error_msg = f"target_fps must be positive, got {target_fps}"
            raise ValueError(error_msg)
        if (target_width is None) != (target_height is None):
            error_msg = "target_width and target_height must be set together"
            raise ValueError(error_msg)
        if target_width is not None and target_width <= 0:
            error_msg = f"target_width must be positive, got {target_width}"
            raise ValueError(error_msg)
        if target_height is not None and target_height <= 0:
            error_msg = f"target_height must be positive, got {target_height}"
            raise ValueError(error_msg)
        if resize_mode != "center-crop":
            error_msg = f"Only center-crop resize_mode is supported, got {resize_mode}"
            raise ValueError(error_msg)

    def _resolution_too_small(self, video: Video) -> bool:
        if self._target_width is None or self._target_height is None:
            return False
        width = video.metadata.width
        height = video.metadata.height
        return width is None or height is None or width < self._target_width or height < self._target_height

    def _mark_video_resolution_too_small(self, video: Video) -> None:
        reason = (
            f"resolution_too_small_for_{self._target_width}x{self._target_height}: "
            f"{video.metadata.width}x{video.metadata.height}"
        )
        logger.warning(f"Filtering {video.input_video}: {reason}")
        for clip in video.clips:
            clip.errors["transcode"] = reason
        video.filtered_clips.extend(video.clips)
        video.clip_stats.num_filtered_by_resolution += len(video.clips)
        video.clips = []

    def _build_video_filter(self) -> str | None:
        filters: list[str] = []
        if self._target_fps is not None:
            filters.append(f"fps={self._target_fps:g}")
        if self._target_width is not None and self._target_height is not None:
            filters.extend(
                [
                    f"crop='min(iw,ih*{self._target_width}/{self._target_height})':"
                    f"'min(ih,iw*{self._target_height}/{self._target_width})'",
    ...
    def _has_video_filter(self) -> bool:
        return self._target_fps is not None or (self._target_width is not None and self._target_height is not None)
    ...
    def _process_video(self, video: Video) -> None:
        ...
        if self._resolution_too_small(video):
            self._mark_video_resolution_too_small(video)
