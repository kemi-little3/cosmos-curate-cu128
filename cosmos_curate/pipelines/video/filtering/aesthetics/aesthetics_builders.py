...
from cosmos_curate.pipelines.common.filter_prompts import get_camera_character_coupling_prompt, get_qwen_filter_prompt
...
from cosmos_curate.pipelines.video.captioning.captioning_builders import QWEN36_OPENAI_EXTRA_BODY
...
from cosmos_curate.pipelines.video.filtering.aesthetics.semantic_filter_stages import (
    VllmCameraCharacterCouplingFilterStage,
    VllmFilteringStage,
...
@attrs.define(frozen=True)
class CameraCharacterCouplingFilterConfig:
    """Configuration for VLM-based camera-character motion coupling filtering."""
...
def _make_vllm_config(
    config: VlmFilterConfig | VideoClassifierConfig | CameraCharacterCouplingFilterConfig,
    prompt_text: str,
) -> VllmConfig:
...
def _make_window_config(config: VlmFilterConfig | VideoClassifierConfig | CameraCharacterCouplingFilterConfig) -> WindowConfig:
...
def _build_external_filter_stages(fc: VlmFilterConfig) -> list[CuratorStage | CuratorStageSpec]:
    ...
    return [
        ApiPrepStage(
            ...
            model_variant=model_variant,
            use_filter_windows=True,
            whole_clip_filter_windows=True,
            verbose=fc.verbose,
            log_stats=fc.perf_profile,
        ),
        caption_stage,
...
def _build_external_camera_character_coupling_filter_stages(
    pc: CameraCharacterCouplingFilterConfig,
) -> list[CuratorStage | CuratorStageSpec]:
    """Build ApiPrepStage + caption stage + VllmCameraCharacterCouplingFilterStage for external endpoints."""
    window_config = _make_window_config(pc)
    prompt_text = get_camera_character_coupling_prompt(verbose=pc.verbose)
    model_variant = pc.endpoint  # "openai" or "gemini"

    caption_stage: CuratorStage
    if pc.endpoint == "openai":
        caption_stage = OpenAICaptionStage(
            model_name=pc.openai_model_name,
            model_variant=model_variant,
            prompt_variant="camera_character_coupled",
            prompt_text=prompt_text,
            max_output_tokens=pc.max_output_tokens,
            use_filter_windows=True,
            endpoint_key="filter",
            max_caption_retries=pc.openai_max_caption_retries,
            retry_delay_seconds=pc.openai_retry_delay_seconds,
            batch_size=pc.batch_size,
            extra_body=QWEN36_OPENAI_EXTRA_BODY,
            verbose=pc.verbose,
            log_stats=pc.perf_profile,
        )
    else:
        caption_stage = GeminiCaptionStage(
            model_variant=model_variant,
            model_name=pc.gemini_model_name,
            prompt_variant="camera_character_coupled",
            prompt_text=prompt_text,
            max_output_tokens=pc.max_output_tokens,
            use_filter_windows=True,
            max_caption_retries=pc.gemini_max_caption_retries,
            retry_delay_seconds=pc.gemini_retry_delay_seconds,
            batch_size=pc.batch_size,
            verbose=pc.verbose,
            log_stats=pc.perf_profile,
        )
    ...
    return [
        ...
        CuratorStageSpec(
            VllmCameraCharacterCouplingFilterStage(
                model_variant=model_variant,
                rejection_threshold=pc.rejection_threshold,
                score_only=pc.score_only,
                verbose=pc.verbose,
                log_stats=pc.perf_profile,
            )
        ),
...
def _build_vllm_camera_character_coupling_filter_stages(
    pc: CameraCharacterCouplingFilterConfig,
) -> list[CuratorStage | CuratorStageSpec]:
    """Build prep + inference + camera-character coupling post-processing stages using VllmCaptionStage."""
    if pc.endpoint != "local":
        return _build_external_camera_character_coupling_filter_stages(pc)
    prompt_text = get_camera_character_coupling_prompt(verbose=pc.verbose)
    vllm_config = _make_vllm_config(pc, prompt_text)
    window_config = _make_window_config(pc)
    return [
        VllmPrepStage(
            vllm_config,
        ...
        CuratorStageSpec(
            VllmCameraCharacterCouplingFilterStage(
                model_variant=pc.model_variant,
                rejection_threshold=pc.rejection_threshold,
                score_only=pc.score_only,
                verbose=pc.verbose,
                log_stats=pc.perf_profile,
...
def build_vllm_filter_classifier_stages(
    ...
    camera_character_coupling_config: CameraCharacterCouplingFilterConfig | None = None,
) -> list[CuratorStage | CuratorStageSpec]:
    ...
    camera_character_coupling_config, and/or classifier_config (at least one required). When
    multiple configs are set, semantic filtering runs first, POV filtering second,
    and classifier filtering last.
    """
    if filter_config is None and classifier_config is None and camera_character_coupling_config is None:
        msg = "At least one of filter_config, camera_character_coupling_config, or classifier_config is required"
        raise ValueError(msg)
    stages: list[CuratorStage | CuratorStageSpec] = []
    if filter_config is not None:
        stages.extend(_build_vllm_filter_stages(filter_config))
    if camera_character_coupling_config is not None:
        stages.extend(_build_vllm_camera_character_coupling_filter_stages(camera_character_coupling_config))
    if classifier_config is not None:
        stages.extend(_build_vllm_classifier_stages(classifier_config))
    return stages
