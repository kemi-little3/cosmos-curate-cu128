...
from cosmos_curate.pipelines.common.semantic_filter_postprocess import (
    ...
    evaluate_camera_character_coupling_window_results,
...
class VllmFilteringStage(CuratorStage):
    ...
    def _process_semantic_clip(
        ...
        clip_should_pass, all_issues, per_window_reasons, per_window_errors, per_window_audit = (
            evaluate_semantic_window_results(
            window_results,
        ...
        for window_idx, audit in per_window_audit.items():
            if "text_type" in audit:
                clip.filter_windows[window_idx].caption["qwen_filter_text_type"] = audit["text_type"]
            if "reason" in audit:
                clip.filter_windows[window_idx].caption["qwen_filter_reason"] = audit["reason"]
...
class VllmCameraCharacterCouplingFilterStage(CuratorStage):
    """CPU post-processing stage for camera-character motion coupling filtering."""
    ...
    def __init__(  # noqa: PLR0913
        ...
        *,
        ...
        """Initialize the camera-character coupling filter stage."""
        self._timer = StageTimer(self)
        self._model_variant = model_variant
        self._rejection_threshold = rejection_threshold
        self._score_only = score_only
    ...
    @nvtx.annotate("VllmCameraCharacterCouplingFilterStage")  # type: ignore[untyped-decorator]
    def process_data(self, tasks: list[SplitPipeTask]) -> list[SplitPipeTask] | None:
        """Filter video clips using camera-character coupling captions on filter windows."""
        for task in tasks:
            self._timer.reinit(self, task.get_major_size())
            video = task.video
            mapping: dict[int, tuple[int, int]] = {}
    ...
    def _filter_clips(
        ...
        passing_clips: list[Clip] = []
        for clip_idx, window_results in clip_results.items():
            self._process_coupling_clip(video, clip_idx, window_results, passing_clips)

        original_clips = list(video.clips)
        for clip_idx in range(len(original_clips)):
            if clip_idx not in clip_results:
                self._handle_unmapped_coupling_clip(original_clips[clip_idx], video, passing_clips)
    ...
    def _process_coupling_clip(
        ...
        clip_should_pass, per_window_values, per_window_errors, yes_fraction = (
            evaluate_camera_character_coupling_window_results(
                window_results,
                rejection_threshold=self._rejection_threshold,
                score_only=self._score_only,
        ...
        clip = video.clips[clip_idx]
        for window_idx, value in per_window_values.items():
            filter_window = clip.filter_windows[window_idx]
            clip.qwen_camera_character_coupled[(filter_window.start_frame, filter_window.end_frame)] = value
            filter_window.caption["qwen_camera_character_coupled"] = value
        for window_idx, error in per_window_errors.items():
            filter_window = clip.filter_windows[window_idx]
            clip.qwen_camera_character_coupled_errors[(filter_window.start_frame, filter_window.end_frame)] = error
            filter_window.errors["qwen_camera_character_coupled"] = error
        ...
        if self._verbose:
            logger.info(
                f"Clip {clip.uuid} filtered out due to camera-character coupling fraction {yes_fraction:.3f}"
            )
        clip.qwen_rejection_stage = "camera_character_coupled"
        video.filtered_clips.append(clip)
        video.clip_stats.num_filtered_by_qwen_camera_character_coupled += 1

    def _handle_unmapped_coupling_clip(self, clip: Clip, video: Video, passing_clips: list[Clip]) -> None:
        """Handle clips that had no successfully mapped coupling-filter windows."""
        clip.errors["qwen_camera_character_coupled"] = "all_windows_failed_preparation"
        if self._score_only:
            passing_clips.append(clip)
            logger.warning(
                f"Clip {clip.uuid} had no successfully mapped coupling windows during score-only mode; kept"
            )
            return
        ...
        video.filtered_clips.append(clip)
        video.clip_stats.num_filtered_by_qwen_camera_character_coupled += 1
        logger.warning(f"Clip {clip.uuid} had no successfully mapped coupling windows; added to filtered_clips")
