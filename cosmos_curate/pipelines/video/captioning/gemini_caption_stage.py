...
class ApiPrepStage(CuratorStage):
    ...
    def __init__(  # noqa: PLR0913
        ...
        *,
        ...
        whole_clip_filter_windows: bool = False,
        verbose: bool = False,
        ...
        self._whole_clip_filter_windows = whole_clip_filter_windows
    ...
    def _prep_windows(self, video: Video) -> None:
        ...
        if self._use_filter_windows and self._whole_clip_filter_windows:
            self._prep_whole_clip_filter_windows(video)
            return
    ...
    def _prep_whole_clip_filter_windows(self, video: Video) -> None:
        """Create one filter window per clip covering the full encoded clip."""
        for clip in video.clips:
            clip.windows = []
            clip.filter_windows = []
            data = clip.encoded_data.resolve()
            if data is None:
                logger.error(f"clip {clip.uuid} does not have encoded_data")
                clip.errors["clip_windowing"] = "clip.encoded_data is None"
                continue
            total_frames = windowing_utils.get_frame_count(data)
            if total_frames <= 0:
                logger.error(f"clip {clip.uuid} has no decodable frames")
                clip.errors["clip_windowing"] = "no decodable frames"
                continue
            clip.filter_windows.append(Window(start_frame=0, end_frame=total_frames - 1, mp4_bytes=data))

        if self._verbose:
            num_windows = sum(len(clip.filter_windows) for clip in video.clips)
            logger.debug(f"Prepared {num_windows} whole-clip filter windows for {video.input_video}")
