...
VLLM_CAPTION_ALGOS: frozenset[str] = frozenset(
    {"nemotron", "qwen", "qwen3_5_27b", "qwen3_6_27b", "qwen3_vl_30b", "qwen3_vl_30b_fp8", "qwen3_vl_235b", "qwen3_vl_235b_fp8"}
    | {"cosmos_r1", "cosmos_r2"}
)

QWEN36_OPENAI_EXTRA_BODY: dict[str, object] = {
    "top_k": 20,
...
def _build_vllm_async_prep_stage(config: CaptioningConfig, vsc: VllmAsyncCaptionConfig) -> CuratorStageSpec:
    ...
    prep_config = VllmAsyncPrepConfig(
        ...
        drop_incomplete_windows=config.window_config.drop_incomplete_windows,
        ...
        case GeminiConfig() as gcfg:
            return CuratorStageSpec(
                GeminiCaptionStage(
                    ...
                    num_cpus_per_worker=gcfg.num_cpus_per_worker,
                    verbose=config.verbose,
                    log_stats=config.perf_profile,
                ),
                num_workers_per_node=gcfg.num_workers_per_node,
        ...
        case OpenAIConfig() as ocfg:
            return CuratorStageSpec(
                OpenAICaptionStage(
                    ...
                    extra_body=ocfg.extra_body,
                    num_cpus_per_worker=ocfg.num_cpus_per_worker,
                    verbose=config.verbose,
                    log_stats=config.perf_profile,
                ),
                num_workers_per_node=ocfg.num_workers_per_node,
                worker_max_lifetime_m=0,
            )
        case VllmAsyncCaptionConfig() as vsc:
...
def build_t5_stages(config: T5Config) -> list[CuratorStage | CuratorStageSpec]:
    ...
    return [
        CuratorStageSpec(
            ...
            worker_max_lifetime_m=config.worker_max_lifetime_m,
        ),
    ]
