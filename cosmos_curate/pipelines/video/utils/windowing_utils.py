...
def select_weighted_window_size(
    total_frames: int,
    config: WeightedFrameWindowConfig,
    clip_uuid: UUID,
) -> int | None:
    """Select one eligible window size for a clip using stable weighted randomness."""
    eligible: list[tuple[int, float]] = [
        (choice, weight)
        for choice, weight in zip(config.choices, config.weights, strict=True)
        if choice <= total_frames
    ]
    if not eligible:
        return None

    digest = hashlib.sha256(f"{config.random_seed}:{clip_uuid}".encode()).digest()
    seed = int.from_bytes(digest[:8], "big", signed=False)
    rng = random.Random(seed)  # noqa: S311 - deterministic sampling, not security-sensitive
    total_weight = sum(weight for _, weight in eligible)
    threshold = rng.random() * total_weight
    cumulative = 0.0
    for choice, weight in eligible:
        cumulative += weight
...
def compute_windows(  # noqa: C901, PLR0911, PLR0913
    total_frames: int,
    window_size: int = 128,
    ...
    *,
    ...
    Args:
        ...
        drop_incomplete_windows: If True, only emit full window_size windows and
            discard trailing frames.
        weighted_frame_window_config: Optional weighted frame-window selection config.
        clip_uuid: Clip UUID used for deterministic weighted frame-window selection.
    ...
    if weighted_frame_window_config is not None:
        if clip_uuid is None:
            msg = "clip_uuid is required when weighted_frame_window_config is set"
            raise ValueError(msg)
        selected_window_size = select_weighted_window_size(total_frames, weighted_frame_window_config, clip_uuid)
        if selected_window_size is None:
            return []
        return [
            WindowFrameInfo(i * selected_window_size, (i + 1) * selected_window_size - 1)
            for i in range(total_frames // selected_window_size)
        ]
    if drop_incomplete_windows:
        if total_frames < window_size:
            return []
        return [WindowFrameInfo(i * window_size, (i + 1) * window_size - 1) for i in range(total_frames // window_size)]
    if total_frames <= window_size:
...
def split_video_into_windows(  # noqa: PLR0913
    ...
    *,
    ...
    Args:
        ...
        drop_incomplete_windows: If True, discard trailing frames that do not
            fill a complete window.
        weighted_frame_window_config: Optional weighted frame-window selection config.
        clip_uuid: Clip UUID used for deterministic weighted frame-window selection.
    ...
    with make_pipeline_named_temporary_file(sub_dir="windowing") as input_file:
        ...
        if return_bytes:
            if len(windows) == 1 and windows[0].start == 0 and windows[0].end == total_frames - 1:
...
def _make_windows_for_clip(  # noqa: PLR0913
    ...
    *,
    ...
    window_mp4_bytes, window_frames, window_infos = split_video_into_windows(
        ...
        drop_incomplete_windows=config.drop_incomplete_windows,
        weighted_frame_window_config=config.weighted_frame_window_config,
        clip_uuid=clip.uuid,
    )
