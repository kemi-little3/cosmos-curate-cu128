...
_IMAGE_BAKED_UIDS = frozenset({0, 1000})
...
def _get_pixi_mount_strings(opts: LaunchDocker) -> list[str]:
    ...
    if not pixi_dir.is_dir():
        ...
        sys.exit(1)
    return ["-v", f"{pixi_dir.absolute()}:{CONTAINER_PATHS_CODE_DIR / '.pixi'}"]
...
def _launch_in_docker_container(opts: LaunchDocker) -> None:
    ...
    docker_command = [
        ...
        "run",
        "--rm",
        *( ["--userns=host"] if os.environ.get("COSMOS_CURATE_LOCAL_DOCKER_USERNS_HOST") == "1" else [] ),
        f"--gpus={gpus_string}",
    ...
    if os.environ.get("COSMOS_CURATE_SKIP_CODE_PIXI_MOUNTS") != "1":
    ...
    local_preamble = os.environ.get("COSMOS_CURATE_LOCAL_PREAMBLE")
    if local_preamble:
        preamble_parts.append(local_preamble)
    if opts.pixi_path is not None:
        ...
        if host_code_dir != CONTAINER_PATHS_CODE_DIR:
            ...
            preamble_parts.append(f"(mkdir -p {host_parent} 2>/dev/null && ln -sfn {container} {host}) || true")
